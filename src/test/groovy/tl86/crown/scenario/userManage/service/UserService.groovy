...
class UserService {
    ...
    def getEncytptPassword(password, loginName) {
    ...
    void addUser(def loginName, def password = "abc123", def roleName = "systemManager") {
        if (!ifUserExist(loginName)) {
            userRepository.insertSysUserTable(loginName, loginName, getEncytptPassword(password, loginName), "test@163.com", "18181971234", 0)
